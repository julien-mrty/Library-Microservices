...
describe('🎬 Movie Controller Tests', () => {
  ...
  test('✅ addMovie should add a movie', async () => {
    const mockReq = { body: { title: 'New Movie', director: 'Jane Doe', year: 2022 } };
    const mockRes = { status: jest.fn().mockReturnThis(), json: jest.fn() };
    await addMovie(mockReq, mockRes);
    expect(mockRes.status).toHaveBeenCalledWith(201);
    expect(mockRes.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Movie added successfully' }));
  });

  test('✅ updateMovie should update an existing movie', async () => {
    const mockReq = { params: { id: movieId }, body: { title: 'Updated Title', director: 'Director', year: 2025 } };
    const mockRes = { json: jest.fn() };
    await updateMovie(mockReq, mockRes);
    expect(mockRes.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Movie updated successfully' }));
  });
